...
function validateForm() {
    ...
    if(coment=="") {
        alert("Debes rellenar el campo del comentario");
        return false;
    }
    ...
    //var z = '<p class="nf">'+usuario.nombre+' comentó el '+ fecha.getDate()+'-'+fecha.getMonth()+'-'+fecha.getFullYear()+' a las '+fecha.getHours()+':'+fecha.getMinutes()+'</p>'+
    //'<p class="txtcom">'+coment+'</p>';
    ...
    for (var i=0; i < arrayCom.length; i++) {
        for(var j=0; j < censuradas.length; j++){
            if(arrayCom[i].toLowerCase() == censuradas[j]){
                arrayCom[i] = arrayCom[i].replace(/\w|ñ/gi,"*");
                cambio = true;
            }
        }
    }
