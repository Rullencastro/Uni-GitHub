<?php
  ...
  function getEvento($idEv) {
    ...
    $stmt = $mysqli->prepare("SELECT id,titulo, subtitulo, fecha, texto, enlace,textoEnlace, twitter, facebook,publicado  FROM eventos WHERE id= ?");
    ...
    if ($res->num_rows > 0) {
      ...
      $evento = array('id' => $row['id'] ,'titulo' => $row['titulo'],'subtitulo' => $row['subtitulo'],'fecha'=> $row['fecha'],'texto'=> $row['texto'],'enlace'=> $row['enlace'],'textoEnlace'=>$row['textoEnlace'] ,'twitter'=> $row['twitter'],'facebook'=> $row['facebook'],'publicado'=> $row['publicado']);
    }
    
    return $evento;
  }

  function eventoPublicado($idEv){
    global $mysqli; 
  ...
  function getImagenesSinFiltro($idEv){
    global $mysqli; 
...
function borrarDatos($nick) {
  ...
  function publicarEvento($idEv){
    ...
    $stmt = $mysqli->prepare("UPDATE eventos SET publicado=true WHERE id=?");
    $stmt->bind_param("i",$idEv);
    $stmt->execute();
  }

  function ocultarEvento($idEv){
    global $mysqli;
    
    $stmt = $mysqli->prepare("UPDATE eventos SET publicado=false WHERE id=?");
    $stmt->bind_param("i",$idEv);
    $stmt->execute();
  }

  function busquedaGeneral($palabra){
    global $mysqli;
    
    $param = "%$palabra%";

    $stmt = $mysqli->prepare("SELECT id,titulo FROM eventos WHERE titulo LIKE ? OR texto LIKE ?");
    ...
    if ($res->num_rows > 0) {
      foreach ($res as $row) {
        $resultados[] = array('id' => $row['id'],'titulo' => $row['titulo']);
      } 
    }
    return $resultados;
  }

  function busquedaPublicado($palabra){
    global $mysqli;
    
    $param = "%$palabra%";

    $stmt = $mysqli->prepare("SELECT id,titulo FROM eventos WHERE publicado=1 AND (titulo LIKE ? OR texto LIKE ?)");
    $stmt->bind_param("ss",$param,$param);
    $stmt->execute();
    $res = $stmt->get_result();
