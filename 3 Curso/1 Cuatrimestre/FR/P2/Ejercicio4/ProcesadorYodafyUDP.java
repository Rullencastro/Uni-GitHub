...
public class ProcesadorYodafyUDP {
	...
	private DatagramPacket datosPacket;
	...
	void procesa(){
		...
		datosPacket = new DatagramPacket(datosRecibidos, datosRecibidos.length);
		...
		try {
			...
			InetAddress ip = datosPacket.getAddress();

			int port = datosPacket.getPort();

			datosPacket = new DatagramPacket(datosEnviar, datosEnviar.length,ip,port);
